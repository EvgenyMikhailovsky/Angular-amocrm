define(['jquery', 'underscore', 'twigjs'], function ($, _, Twig) {
  var CustomWidget = function () {
    ...
    this.getTemplate = _.bind(function (template, params, callback) { //Сбор информации из карточки контакта
    ...
    this.callbacks = { //метод для отображения виджета 
      render: function () {
        console.log('render');
        return true; //необходимо, чтобы вернул true
      },
      init: _.bind(function () { // собирает информацию, связь с сторонним сервером и авторизация по API
        console.log('init');
        ...
        return true; // callbacks.init должен всегда возвращать true 
      }, this),
      bind_actions: function () { // Используеся для навешивания событий на действия пользователя
        console.log('bind_actions');
        return true; // bind_actions должен возвращать true
      },
      settings: function () { // вызывается при щелчке на иконку виджета в области настроек.Может использоваться для добавления на страницу модального окна
        return true;
      },
      onSave: function () { //вызывается при щелчке пользователя на кнопке “Установить/Сохранить” в настройках виджета. Можно использовать для отправки введенных в форму данных и смены статуса виджета
        alert('click');
        return true;
      },
      destroy: function () { //Данная функция вызывается при отключении виджета через меню его настроек.
       ...
       * @return {{}}
       */
      onSalesbotDesignerSave: function (handler_code, params) { //Данная функция определяет логику работы действия виджета в Salesbot и вызывается, если был добавлен виджет в конструкторе Salesbot, при сохранении.
        var salesbot_source = {
            question: [],
            require: []
          },
